...
public class QueueLinkedList1 {
    NodeAntrian1 front, rear; 
    int size;
    int maxCapacity; 

    public QueueLinkedList1(int maxCapacity) {
        front = rear = null;
        size = 0;
    ...
    public void enqueue(MahasiswaAntrian1 data) {
        if (isFull()) {
            System.out.println("Yah, antrian udah penuh nih!");
            return;
        }
        NodeAntrian1 newNode = new NodeAntrian1(data, null);
        if (isEmpty()) {
            front = rear = newNode;
        } else {
            rear.next = newNode;
